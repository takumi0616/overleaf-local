...
\abstract{
Self-Organizing Maps (SOM) based on Euclidean distance, which have been dominant in synoptic pattern classification, struggle to capture structural features of spatial fields. This study applies to SOM a structural similarity metric that integrates the Structural Similarity Index (SSIM) with the meteorology-specific Teweles–Wobus score (S1), and evaluates classification performance for sea level pressure (SLP) over Japan. The fused S1+SSIM measure achieved higher macro recall than the conventional Euclidean distance. The proposed structure-aware approach provides a new framework for objective synoptic pattern classification.
}
\keywords{
