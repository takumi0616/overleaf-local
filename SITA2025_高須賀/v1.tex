...
\abstract{
Self-Organizing Maps (SOM) with EUCLIDEAN often fail to capture structural characteristics of spatial fields. This study implements and compares eight activation distances for SOM-based synoptic pattern classification over Japan: existing single methods S1, SSIM5, EUCLIDEAN; proposed single methods KAPPA, GSSIM; and proposed composite methods S1,KA (impl: s1k), GS,KA (impl: gk), S1,GS,KA (impl: s1gk). On a unified GPU-optimized batch-SOM framework, we evaluate generalization by Macro Recall (base labels only) and a nodewise medoid–majority consistency rate. The framework outputs medoids instead of centroids to preserve sharp representative patterns. This paper documents the exact mathematical definitions used and the experimental protocol; quantitative figures are omitted here and are available in the generated logs/CSV.}
\keywords{
...
本研究は，Batch-SOMにおいて，既存手法（単一手法）S1，SSIM5，EUCLIDEANと，提案手法（単一手法）KAPPA，GSSIM，および提案手法（複合手法）S1,KA（実装名：s1k），GS,KA（実装名：gk），S1,GS,KA（実装名：s1gk）の計8手法を同一条件で厳密比較する．SOMの代表としてcentroidではなくmedoid（true medoid）を出力して解釈性を高め，学習（1991–1997）と独立検証（1998–2000）の分離により汎化性能を評価する．評価は基本ラベル（15種）のMacro Recallと，ノード多数決（基本ラベル）とmedoid（基本ラベル）の一致率で行う．
