...
const upsertFile = wrapWithLock({
  ...
  async withLock({
    ...
    let element
    logger.debug({ projectId, folderId }, 'upsertFile.beforeLock findElement start')
    try {
      ...
      logger.debug({ projectId, folderId, found: Boolean(element) }, 'upsertFile.beforeLock findElement done')
    } catch (error) {
      logger.warn({ projectId, folderId, err: error }, 'upsertFile.beforeLock findElement error')
      if (error instanceof Errors.NotFoundError) {
        throw new Error('folder_not_found')
      }
      throw error
