...
function read_mongo_version() {
  ...
  # normalize in case read_configuration returned full 'KEY=value' lines
  if [[ "$mongo_image" == MONGO_IMAGE=* ]]; then
    mongo_image="${mongo_image#MONGO_IMAGE=}"
  fi
  if [[ "$mongo_version" == MONGO_VERSION=* ]]; then
...
function read_variable() {
  local name=$1
  local line
  line=$(grep -E "^$name=" "$TOOLKIT_ROOT/config/variables.env" | head -n1 || true)
  line=${line#*=}
...
function read_configuration() {
  local name=$1
  local line
  line=$(grep -E "^$name=" "$TOOLKIT_ROOT/config/overleaf.rc" | head -n1 || true)
  line=${line#*=}
  # strip optional surrounding single or double quotes
  if [[ "$line" == \"*\" && "$line" == *\" ]]; then
    line="${line%\"}"
