...
def evaluate_clusters_only_base(clusters: List[List[int]],
    ...
    # 複合ラベル考慮（基本+応用）
    # Composite/transition evaluation removed; compute only base metrics
    macro_recall_composite = float('nan')
    ...
    # Composite Macro Recall removed
...
def log_som_recall_by_label_with_nodes(
    ...
    # Composite/transition metrics removed; base-only evaluation
    ...
    # Composite/transition recall removed
...
def compute_node_true_medoids(
    ...
    - method_name: 'euclidean' | 'ssim5' | 's1' | 'gssim' | 'kappa' | 's1k' | 'gk' | 's1gk'
    ...
    for ix in range(som_shape[0]):
        for iy in range(som_shape[1]):
            ...
            if method_name == 'euclidean':
                ...
                # G-SSIM と Kappa（Kappaを行方向min–max正規化してRMS合成）
                Dg = pairwise_by_ref(lambda Xb, ref: _gssim_dist_to_ref(Xb, ref), Xcand)   # (C,C)
                Dk = pairwise_by_ref(lambda Xb, ref: _kappa_to_ref(Xb, ref), Xcand)        # (C,C)
                eps = 1e-12
                dk_min = Dk.min(dim=1, keepdim=True).values
                dk_max = Dk.max(dim=1, keepdim=True).values
                Dkn = (Dk - dk_min) / (dk_max - dk_min + eps)
                D = torch.sqrt((Dg * Dg + Dkn * Dkn) / 2.0)
...
def compute_nodewise_match_rate(
    ...
    """
    ノード代表（多数決）の基本ラベルと medoid の基本ラベルの一致率（基本のみ）。
    - 代表rawが複合/移行型（+/- を含む）なら None とする
    - medoid raw が複合/移行型なら None とする
    - 代表または medoid が None でも分母に計上（失敗としてカウント）
    戻り値: (match_rate, matched_nodes, counted_nodes)
    """
    import unicodedata

    def is_composite_or_transition(raw: Optional[str]) -> bool:
        if raw is None:
            return False
    ...
    for key, mi in node_to_medoid_idx.items():
        maj_raw = node_to_majority_raw.get(key, None)
        med_raw = labels_all[mi] if labels_all is not None else None

        # 代表（基本）の決定：複合/移行が勝った場合は None
        rep_base = None
        if maj_raw is not None and not is_composite_or_transition(maj_raw):
            rep_base = basic_label_or_none(maj_raw, BASE_LABELS)

        # medoid（基本）：複合/移行なら None
        med_base = None
        if med_raw is not None and not is_composite_or_transition(med_raw):
            med_base = basic_label_or_none(med_raw, BASE_LABELS)

        counted += 1
        if (rep_base is not None) and (med_base is not None) and (rep_base == med_base):
            matched += 1
...
def evaluate_verification_with_training_majority(
    ...
    total_base = Counter()
    correct_base = Counter()
    # Composite/transition counters removed
    ...
    logger.write("\n【各ラベルの再現率（学習時の代表基本ラベルを予測として・基本のみ）】\n")
...
def run_one_method_learning(method_name, activation_distance, data_all, labels_all, times_all,
    ...
    iter_history: Dict[str, List[float]] = {
        ...
        'QuantizationError': [],
        'NodewiseMatchRate': []  # 代表（基本）vs medoid（基本）の一致率
    }
    ...
    for seg in range(SOM_EVAL_SEGMENTS):
        ...
        if metrics is not None and 'MacroRecall_majority' in metrics:
            log.write(f"  MacroRecall_majority = {metrics['MacroRecall_majority']:.6f}\n")
        # 追加: match rate
        ...
        if metrics is not None:
            ...
            iter_history['MacroRecall_majority'].append(np.nan)
    ...
    if labels_all is not None:
        ...
        if metrics is not None:
            log.write('\n[Final Metrics]\n')
            if 'MacroRecall_majority' in metrics:
                log.write(f"  MacroRecall_majority = {metrics['MacroRecall_majority']:.6f}\n")
            # 追加：match rate
        ...
        log_som_recall_by_label_with_nodes(
            ...
            base_labels=BASE_LABELS,
            som_shape=(SOM_X, SOM_Y),
            section_title='SOM代表ノード群ベースの再現率（基本のみ）'
        )
    else:
